...
window.HsOrgChartTemplates.createAreaCardTemplate = function (areaIndex, layoutJson, areaWidth, areaHeight) {
  ...
  function drawConnector(x1, y1, x2, y2) {
    return '' +
      '<path stroke="#dbe5f0" stroke-width="6" fill="none" stroke-linecap="butt" stroke-linejoin="miter" d="M' + x1 + ' ' + y1 + ' L' + x2 + ' ' + y2 + '"></path>' +
      '<path stroke="#9fb6cc" stroke-width="2.6" fill="none" stroke-linecap="butt" stroke-linejoin="miter" d="M' + x1 + ' ' + y1 + ' L' + x2 + ' ' + y2 + '"></path>'
  }
  ...
  if (!hideHeaderLeader) {
    ...
    node +=
      drawConnector(centerX, 56, centerX, headerPersonY) +
  ...
  if ((layout.roots || []).length && !hideRootConnector) {
    ...
    var headerLeaderBottomY = 82 + HS_PERSON_CARD_HEIGHT
