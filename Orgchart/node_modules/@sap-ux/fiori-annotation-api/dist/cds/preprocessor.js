...
class ChangePreprocessor {
     ...
     */
    handleAnnotationInsert(change, parent, i) {
     ...
     * @param i - index in input array
     */
    handleEmbeddedAnnotationInsert(change, parent, grandParent, i) {
     ...
     */
    resolveCompoundTarget(change, parent, grandParent, path) {
     ...
     * @param i - index in input array
     */
    applyCompoundAnnotationCommand(change, target, pointer, i) {
        // make sure that the assignment isn't already a compound assignment
        const startPosition = target.assignments[target.assignments.length - 1]?.range?.start ??
            target.range?.start;
        if (!startPosition) {
            return;
        }
    ...
    expandToCompoundAnnotations() {
        ...
        for (let i = this.input.length - 1; i >= 0; i--) {
            ...
            if (!handledAssignments.has(pointer)) {
                ...
                this.applyCompoundAnnotationCommand(change, target, pointer, i);
            }
        }
    }
